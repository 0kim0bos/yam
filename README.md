...
- Release report JSON: `yam release report --json` summarizes typecheck, forbidden-name scan, package boundary, registry status, CLI smoke, dist freshness, diagnostics, readiness receipt, and final truth status. Default `--auth-mode auto` selects Trusted Publisher OIDC when the reviewed release workflow is present, treats `npm whoami` as an optional manual-token probe rather than OIDC evidence, and blocks on missing workflow controls or unverified npmjs package binding instead. `--auth-mode token` explicitly selects direct local token readiness.
...
- Publish readiness evidence: release reports run read-only registry/auth/version probes, redact account/token details, keep npm publish outside the report, and include a `yam.release-readiness-receipt.v1` basis for the readiness judgment. OIDC account binding remains explicitly unverified until the npmjs Trusted Publisher configuration and hosted publish result provide evidence; this follows the [official npm Trusted Publisher behavior](https://docs.npmjs.com/trusted-publishers/), where `npm whoami` does not reflect OIDC authentication.
